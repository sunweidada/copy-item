...
<script>
import EmployeeEnum from '@/api/constant/employees' // 自己维护的枚举
import {
  getPersonalDetail,
  updatePersonal,
  saveUserDetailById
  ...
  methods: {
    async getPersonalDetail() {
      ...
      if (this.formData?.staffPhoto.trim()) {
        this.$refs.baseInfoRef.fileList = [
          {
    ...
    async savePersonal() {
      // 要先判断一下是否上传了头像
      const fileList = this.$refs.baseInfoRef.fileList
      if (!fileList.length) {
        return this.$message.warning('请上传员工头像！')
      }
    ...
    async saveUser() {
      ...
      // 硬性要求：我们要让用户必须上传头像
      const fileList = this.$refs.userInfoRef.fileList

      // fileList
      // if (!fileList.length) {
      const flag = fileList.some((item) => !item.upload)
      if (flag) {
        return this.$message.warning('请上传员工照片！')
      }
      // 保证 fileList 是有数据
    ...
    async getUserDetailById() {
      ...
      if (this.userInfo?.staffPhoto) {
        // 可以显示头像
        this.$refs.userInfoRef.fileList = [
          {
